...
function calculateTrendingCrops(orders, listings) {
  ...
  return Object.entries(cropTrends)
    ...
    .slice(0, 5)
    .map(([cropName, count]) => {
      const relatedListings = listings.filter(l => (l.milletType || l.cropType) === cropName);
      const avgPrice = relatedListings.length > 0
        ? relatedListings.reduce((sum, l) => sum + (l.pricePerKg || 0), 0) / relatedListings.length
        : 0;
      ...
      return {
        ...
        growthPercentage: Math.round((count / Math.max(orders.length, 1)) * 100),
...
function calculateMostSoldCrops(orders, listings) {
  ...
  return Object.entries(cropSales)
    ...
    .map(([cropName, quantity], index) => {
      const relatedOrders = orders.filter(o => (o.milletType || o.cropType) === cropName);
...
function calculatePriceAnalysis(listings, priceHistory) {
  ...
  return {
    overallAvgPrice: prices.length > 0
      ? Math.round((prices.reduce((a, b) => a + b, 0) / prices.length) * 100) / 100
      : 0,
    highestPrice: Math.max(...prices, 0),
    lowestPrice: Math.min(...prices.filter(p => p > 0), 0),
    priceRange: Math.max(...prices, 0) - Math.min(...prices.filter(p => p > 0), 0),
    cropPriceTrends: priceTrends.sort((a, b) => b.currentAvgPrice - a.currentAvgPrice)
  };
}
...
function calculateMarketVolatility(priceHistory) {
  ...
  const volatilityAnalysis = Object.entries(volatilityByProduct).map(([product, prices]) => {
  ...
  const sortedByVolatility = volatilityAnalysis.sort((a, b) => b.volatility - a.volatility);

  return {
    overallVolatility: Math.round(
      volatilityAnalysis.reduce((sum, p) => sum + p.volatility, 0) / volatilityAnalysis.length * 100
    ) / 100,
...
function calculateTopFarmers(orders, listings) {
  ...
  orders.forEach(order => {
    ...
    farmerStats[farmerId].totalRevenue += (order.quantity || 0) * (order.pricePerKg || 0);
    farmerStats[farmerId].crops.add(order.milletType || order.cropType);
  });

  listings.forEach(listing => {
    if (listing.farmerId && farmerStats[listing.farmerId]) {
      farmerStats[listing.farmerId].name = listing.farmerName || listing.farmerId;
    }
  });
...
function calculateRegionalInsights(orders, listings) {
  ...
  orders.forEach(order => {
    ...
    regionData[region].totalValue = (regionData[region].totalValue || 0) + 
...
function calculateSummary(orders, listings) {
  ...
  const totalRevenue = activeOrders.reduce((sum, o) => 
    sum + ((o.quantity || 0) * (o.pricePerKg || 0)), 0
  );
