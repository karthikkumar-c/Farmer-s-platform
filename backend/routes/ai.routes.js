 ...
 * 4. POST /api/ai/smart-match - Find products matching consumer preferences
 */
 ...
 * Find products matching consumer quality and price preferences
 *
 * Body: {
 *   maxPrice: number (max price per kg willing to pay)
 *   preferredQuality: "Premium" | "Standard" | "Basic"
 *   milletTypes: string[] (optional, e.g., ["Finger Millet", "Pearl Millet"])
...
router.post("/smart-match", async (req, res) => {
  try {
    ...
    // Validate required fields
    if (!preferences.maxPrice || !preferences.preferredQuality) {
      return res.status(400).json({
        error: "Missing required fields",
        required: ["maxPrice", "preferredQuality"],
      });
    }

    // Get smart product matches
    const matches = await getSmartProductMatches(preferences);
    res.json(matches);
  } catch (error) {
