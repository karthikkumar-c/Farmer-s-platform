...
export async function getListingsByFarmer(
  ...
  console.log(
    "[Firestore] getListingsByFarmer called with farmerId:",
    farmerId,
  );
  try {
    const q = query(
    ...
    console.log(
      "[Firestore] getListingsByFarmer query succeeded, found",
      results.length,
      "listings",
    );
    console.log(
      "[Firestore] Listing details:",
      results.map((r) => ({
        id: r.id,
        farmerId: r.farmerId,
    ...
    console.log(
      "[Firestore] getListingsByFarmer query failed, falling back to client-side filter. Error:",
      error,
    );
    const snap = await getDocs(collection(db, "listings"));
    const allListings = snap.docs.map((d) => ({
      id: d.id,
      farmerId: d.data().farmerId,
      status: d.data().verificationStatus,
    }));
    console.log(
      "[Firestore] All listings in database:",
      allListings.length,
      "total",
    );
    console.log("[Firestore] Filtering for farmerId:", farmerId);
    const filtered = snap.docs
      .filter((d) => d.data().farmerId === farmerId)
      .map((d) => ({ id: d.id, ...d.data() }) as Listing);
    console.log(
      "[Firestore] After filtering, found",
      filtered.length,
      "listings for this farmer",
    );
    return filtered;
  }
}
...
export async function createListing(
  ...
  console.log(
    "[Firestore] createListing called with data:",
    JSON.stringify({
  ...
  console.log("[Firestore] createListing succeeded with ID:", docRef.id);
  return docRef.id;
}

export async function deleteListing(id: string): Promise<void> {
  console.log("[Firestore] deleteListing called for ID:", id);
  await deleteDoc(doc(db, "listings", id));
  console.log(
    "[Firestore] Listing deleted successfully from database. It will no longer appear on consumer side.",
  );
}
...
export async function getFarmerDashboardStats(farmerId: string) {
  try {
    const listingsSnap = await getDocs(
      query(collection(db, "listings"), where("farmerId", "==", farmerId)),
    );
    const ordersSnap = await getDocs(
      query(collection(db, "orders"), where("sellerId", "==", farmerId)),
    );
    const paymentsSnap = await getDocs(
      query(collection(db, "payments"), where("farmerId", "==", farmerId)),
    );
...
export async function getPendingVerifications(
  ...
  console.log("[Firestore] getPendingVerifications called for taluks:", taluks);
  try {
    const snap = await getDocs(collection(db, "listings"));
    ...
    console.log(
      "[Firestore] Found",
      results.length,
      "pending listings for SHG verification",
    );
    console.log(
      "[Firestore] Pending listing details:",
      results.map((r) => ({
        id: r.id,
...
export async function getVerifiedByShg(shgId: string): Promise<Listing[]> {
  try {
    const snap = await getDocs(
      query(collection(db, "listings"), where("verifiedBy", "==", shgId)),
    );
...
export async function verifyListing(
  ...
  console.log(
    "[Firestore] verifyListing called - changing status from PENDING to",
    status.toUpperCase(),
  );
  console.log(
  ...
  console.log("[Firestore] Listing verification status updated to:", status);
  console.log(
    "[Firestore] This listing will",
    status === "verified" ? "NOW APPEAR" : "NOT APPEAR",
    "on consumer browse crops page",
  );
  ...
  console.log(
    "[Firestore] Verification record created in verifications collection",
  );
}
...
export async function getVerificationHistory(
  ...
  try {
    const snap = await getDocs(
      query(collection(db, "verifications"), where("shgId", "==", shgId)),
    );
...
export async function getSHGDashboardStats(shgId: string, taluks: string[]) {
  try {
    ...
    const listings = allListings.docs.map((d) => ({
      id: d.id,
      ...d.data(),
    })) as Listing[];
    const pending = listings.filter(
      (l) => l.verificationStatus === "pending" && taluks.includes(l.taluk),
    );
    const verified = listings.filter(
      (l) => l.verifiedBy === shgId && l.verificationStatus === "verified",
    );
    const rejected = listings.filter(
      (l) => l.verifiedBy === shgId && l.verificationStatus === "rejected",
    );
    return {
...
export async function getVerifiedListings(): Promise<Listing[]> {
  console.log(
    "[Firestore] getVerifiedListings called - fetching only VERIFIED listings for consumers",
  );
  try {
    const snap = await getDocs(
      query(
    ...
    console.log(
      "[Firestore] Found",
      results.length,
      "verified listings for consumers",
    );
    return results;
  } catch {
    console.log("[Firestore] Query failed, falling back to client-side filter");
    const snap = await getDocs(collection(db, "listings"));
    const results = snap.docs
      .map((d) => ({ id: d.id, ...d.data() }) as Listing)
      .filter(
    ...
    console.log(
      "[Firestore] Found",
      results.length,
      "verified listings (client-side filter)",
    );
    return results;
  }
}
...
export async function getConsumerDashboardStats(consumerId: string) {
  try {
    ...
    const ordersSnap = await getDocs(
      query(collection(db, "orders"), where("buyerId", "==", consumerId)),
    );
    const orders = ordersSnap.docs.map((d) => d.data());
    return {
      ...
      .map((o) => ({
        ...
        timeline: [
          ...
          },
          {
            ...
            date: o.status === "placed" ? "Pending" : "Done",
            time: "",
          },
          {
            status: "Shipped",
            completed: ["shipped", "delivered"].includes(o.status),
            date: ["shipped", "delivered"].includes(o.status)
              ? "Done"
              : "Pending",
            time: "",
          },
          {
            ...
            date: o.status === "delivered" ? "Done" : "Pending",
            time: "",
          },
        ],
      }));
...
export async function getAnalyticsData() {
  try {
    ...
    listings.forEach((l) => {
      milletCounts[l.milletType] = (milletCounts[l.milletType] || 0) + 1;
    });
    ...
    const milletDistribution = Object.entries(milletCounts).map(
      ([name, value], i) => ({
        name,
    ...
    return {
      ...
      userGrowth: 12,
      verifiedCrops: listings.filter((l) => l.verificationStatus === "verified")
        .length,
      cropGrowth: 22,
      totalOrders: orders.length,
      orderGrowth: 15,
      monthlyData: [
        { month: "Jul", farmers: 10, orders: 5 },
        { month: "Aug", farmers: 15, orders: 8 },
        { month: "Sep", farmers: 20, orders: 12 },
        { month: "Oct", farmers: 25, orders: 18 },
        { month: "Nov", farmers: 30, orders: 22 },
        {
          month: "Dec",
      ...
      regionData: [
        {
          ...
          farmers: users.filter(
            (u) => u.role === "farmer" && u.district === "Bengaluru Rural",
          ).length,
        },
        {
          region: "Tumkur",
          farmers: users.filter(
            (u) => u.role === "farmer" && u.district === "Tumkur",
          ).length,
        },
        {
          region: "Mysuru",
          farmers: users.filter(
            (u) => u.role === "farmer" && u.district === "Mysuru",
          ).length,
        },
        {
          region: "Hassan",
          farmers: users.filter(
            (u) => u.role === "farmer" && u.district === "Hassan",
          ).length,
        },
        {
          region: "Mandya",
          farmers: users.filter(
            (u) => u.role === "farmer" && u.district === "Mandya",
          ).length,
        },
      ],
    };
